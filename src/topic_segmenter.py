...
from src.config import TOPIC_SIMILARITY_THRESHOLD, TOPIC_SEGMENT_OVERLAP, EMBEDDING_MODEL
from src.logging_utils import get_logger

# Get logger
logger = get_logger(__name__)

# Singleton model instance - loaded once at module level
    ...
    def segment(self, text: str) -> List[Dict]:
        ...
        for i in range(1, len(embeddings)):
            ...
            if sim < self.threshold and len(current_sentences) >= 2:
                ...
                overlap_sentences = current_sentences[-self.overlap:] if len(current_sentences) >= self.overlap else []
