...
def validate_file_size(file_bytes: bytes) -> Tuple[bool, str]:
    ...
    if len(file_bytes) > MAX_FILE_SIZE:
        return False, f"Error: File too large. Maximum size is {MAX_FILE_SIZE // (1024*1024)}MB"
    return True, ""
...
def process_file(uploaded_file) -> Tuple[str, str]:
    ...
    try:
        ...
        return filename, "Error: Unable to decode file. Please use UTF-8 encoded text files."
    except Exception as e:
